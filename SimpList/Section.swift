...
class Section: NSObject, NSCoding {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        // The items array is required. If we cannot decode an items array, the initializer will fail.
        guard let items = aDecoder.decodeObject(forKey: PropertyKey.items) as? [Item] else {
            os_log("Unable to decode the items array for the Section object.", log: OSLog.default, type: .debug)
            return nil
        }
